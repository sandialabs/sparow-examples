...
exactly as already used in the EF runners.
...
# Matches the patterns used in setup_load_scens.py / scale_tests create_sp.
...
def _try_import_create_gtep_model():
    ...
    -----------------------------------------------
    data/ does NOT live next to this staged script.
    It lives under the model tree that gets copied by setup_load_scens:

        model/data/                         # template
        scale_tests/single/data/            # after setup_load_scens.py
    ...
    candidates = [
        # Generated scale-tests package (preferred after setup_load_scens)
        "sparow_examples.aos_gtep_9bus.scale_tests.single.driver_gtep",
        "scale_tests.single.driver_gtep",
        # Template model tree
    ...
    for rel in (
        here / "scale_tests" / "single" / "driver_gtep.py",
        here / "model" / "driver_gtep.py",
        Path.cwd() / "scale_tests" / "single" / "driver_gtep.py",
    ...
    raise ImportError(
        ...
        "scale_tests/single/data/), NOT next to this staged script.\n\n"
        "Tried:\n  " + "\n  ".join(errors) + "\n\n"
        "Fix: run  python setup_load_scens.py  once so scale_tests/single/ exists\n"
...
def stage8_run_benders(
    ...
    via the BendersSolver.solve_and_return_model(subproblem_transforms=...) hook::

        subproblem_transforms=[relax_second_stage]

    Master does NOT receive residual relaxation (first-stage stay discrete).

    Uses the public SPAROW API only:
    - solve_and_return_model (returns live master + solutions)
    - on_iteration callback for progress banners (no generate_cut monkey-patch)

    convergence_tol is the absolute OR-TOPAS cut-acceptance / θ-difference
    threshold passed as solve_and_return_model(..., convergence_tol=...).
    Default 1e-3 is appropriate for GTEP-scale objectives (~1e7+); the SPAROW
    method default of 1e-8 is typically too tight for these models.
    ...
    _print(f"  convergence_tol (OR-TOPAS cut/θ threshold): {convergence_tol}")
    ...
    # Progress banners via the public on_iteration callback (no monkey-patch)
    _iter_state = {"cuts_total": 0, "last_iter": 0}

    def _on_iteration(data):
        _iter_state["last_iter"] = data.iter_idx
        n_cuts = len(data.cuts_added) if data.cuts_added is not None else 0
        _iter_state["cuts_total"] += n_cuts
        master_obj = None
        eta_vals = None
        try:
            m = getattr(data, "upper_model", None)
            ...
            pass
        parts = [
            f"  --- Benders iteration {data.iter_idx}",
            f"cuts_added={n_cuts}",
    ...
    if hasattr(solver, "solve_and_return_model"):
        ...
        results = result.solutions
        upper_model = getattr(result, "upper_model", None)
    else:
        # Defensive fallback for older SPAROW builds that only expose solve()
    ...
    _print(
        f"  Solve returned after {_iter_state.get('last_iter', 0)} Benders "
        f"iteration(s), {_iter_state['cuts_total']} cut(s) total."
    )
