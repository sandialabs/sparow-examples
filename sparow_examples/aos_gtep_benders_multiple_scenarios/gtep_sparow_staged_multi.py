...
Adapted for aos_gtep_benders_multiple_scenarios.

Two scenarios:
  load_0p95  → alpha=0.95, Probability=0.5
  load_1p05  → alpha=1.05, Probability=0.5

Requires the package produced by setup_load_scens.py::

    scale_tests/
      load_0p95/    # model copy + data/ + __init__.py
      load_1p05/
      __init__.py   # create_sp() with Rung-2b defaults

Also requires the single-scenario sibling `gtep_sparow_staged.py` to be
importable (same directory or on PYTHONPATH) for the shared stage helpers.
...
write missing __init__.py markers under scale_tests/* as a fallback.

Stage map (same ladder as single)
---------------------------------
0–4  Environment, one concrete model (from load_0p95 tree), inventory, trivial SP
...
IMPORTANT: EF may rebundle the two scenarios into a single bundle. Classical
Benders asserts one scenario per bundle, so Stages 7–8 rebuild a fresh SP.

Usage (from aos_gtep_benders_multiple_scenarios/, after setup_load_scens.py)
---------------------------------------------------------------------------
    python -c "
    ...
    run_staged_diagnostics_multi(
        ...
        stop_after_ef=True,
        run_benders=False,
        package_name='scale_tests',
    )
    " 2>&1 | tee staged_multi_ef.log
    ...
    " 2>&1 | tee staged_multi_benders.log

After residual Binary ≈ 0 and Benders matches EF, re-run with force_minimal=False
and Rung 2b sizes (stages=2, num_rep_days=1, len_rep_days=12, num_commit_p=8, num_disp=1).
"""
...
def _ensure_package_roots_on_path():
    ...
    Ensure the directory that *contains* scale_tests/ (or model/) is on sys.path.
    """
...
def _ensure_scenario_package_markers(pkg_root: Path):
    ...
    for init_dir in (
        ...
        pkg_root / PACKAGE_NAME / "load_0p95",
        pkg_root / PACKAGE_NAME / "load_1p05",
    ):
        if init_dir.is_dir():
...
def _try_import_create_gtep_model_multi():
    """
    Prefer create_gtep_model from scale_tests/<scenario>/ so data/ resolves
    under the multi-scenario package produced by setup_load_scens.py.
    ...
    candidates = [
        f"{PACKAGE_NAME}.load_0p95.driver_gtep",
        f"{PACKAGE_NAME}.load_1p05.driver_gtep",
        f"sparow_examples.aos_gtep_benders_multiple_scenarios.{PACKAGE_NAME}.load_0p95.driver_gtep",
        f"sparow_examples.aos_gtep_benders_multiple_scenarios.{PACKAGE_NAME}.load_1p05.driver_gtep",
        f"aos_gtep_benders_multiple_scenarios.{PACKAGE_NAME}.load_0p95.driver_gtep",
        f"aos_gtep_benders_multiple_scenarios.{PACKAGE_NAME}.load_1p05.driver_gtep",
        f"sparow_examples.gtep_9bus_benders.{PACKAGE_NAME}.load_0p95.driver_gtep",
        f"sparow_examples.gtep_9bus_benders.{PACKAGE_NAME}.load_1p05.driver_gtep",
        f"gtep_9bus_benders.{PACKAGE_NAME}.load_0p95.driver_gtep",
        f"gtep_9bus_benders.{PACKAGE_NAME}.load_1p05.driver_gtep",
        f"sparow_examples.aos_gtep_9bus.{PACKAGE_NAME}.load_0p95.driver_gtep",
        f"sparow_examples.aos_gtep_9bus.{PACKAGE_NAME}.load_1p05.driver_gtep",
        f"aos_gtep_9bus.{PACKAGE_NAME}.load_0p95.driver_gtep",
        f"aos_gtep_9bus.{PACKAGE_NAME}.load_1p05.driver_gtep",
        # fall backs
        "model.driver_gtep",
    ...
    # Package-aware fallback: put parent of scale_tests on path, ensure
    # __init__.py markers, then import as scale_tests.load_0p95.driver_gtep
    here = Path(__file__).resolve().parent
    cwd = Path.cwd()
    for pkg_root in (cwd, here):
        scen_dir = pkg_root / PACKAGE_NAME / "load_0p95"
        ...
        mod_name = f"{PACKAGE_NAME}.load_0p95.driver_gtep"
        try:
    ...
    raise ImportError(
        ...
        f"Run  python setup_load_scens.py  so {PACKAGE_NAME}/load_0p95/data exists.\n"
        "driver_gtep uses relative imports; the package root that contains "
        f"{PACKAGE_NAME}/ must be on PYTHONPATH (usually the cwd).\n"
        "Also ensure:\n"
        f"  touch {PACKAGE_NAME}/load_0p95/__init__.py {PACKAGE_NAME}/load_1p05/__init__.py\n"
        "Tried:\n  " + "\n  ".join(errors)
    )
...
def stage5_gtep_sparow_sp_multi(
    ...
    *,
    ...
    def gtep_builder(data, args):
        ...
        scen_id = data.get("ID", "load_0p95")

        mod = None
        for mod_name in (
            f"{package_name}.{scen_id}",
            f"sparow_examples.aos_gtep_benders_multiple_scenarios.{package_name}.{scen_id}",
            f"aos_gtep_benders_multiple_scenarios.{package_name}.{scen_id}",
...
def run_staged_diagnostics_multi(
    *,
    ...
    Default scenarios::
        ...
        load_0p95  alpha=0.95  P=0.5
        load_1p05  alpha=1.05  P=0.5
    """
    _print("=" * 72)
    ...
    # load_0p95 + load_1p05 into one bundle with two scenarios.
