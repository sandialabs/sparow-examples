...
def HF_PH():
    ...
    sp.initialize_model(
        name="HF", model_data=model_data_noisey["HF"], model_builder=HF_builder
    )
    sp.initialize_model(
        name="LF", model_data=model_data_noisey["LF"], model_builder=LF_builder_SHED
    )
    bundle_num = 0
    
    sp.initialize_bundles(
        scheme="mf_kmeans_similar",data_key='DEMAND'
    )
