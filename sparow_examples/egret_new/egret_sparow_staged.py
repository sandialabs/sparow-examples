...
def _truncate_model_data_horizon(md: "ModelData", n_periods: int) -> "ModelData":
    ...
    md = copy.deepcopy(md)
    if "system" not in md.data or "time_keys" not in md.data.get("system", {}):
        raise KeyError(
            "ModelData missing data['system']['time_keys'] after load. "
            "If this is a PGLIB-UC file, ensure it was read with "
            "file_type='pglib-uc' (see _read_egret_model_data)."
        )
...
def _read_egret_model_data(path: str):
    """
    Load ModelData from path, auto-detecting PGLIB-UC vs native EGRET JSON.

    PGLIB-UC files (e.g. rts_gmlc/*.json) use keys like time_periods /
    thermal_generators and must be read with file_type='pglib-uc'.  Native
    EGRET ModelData JSONs already have system / elements.
    """
    import json
    from egret.data.model_data import ModelData
    ...
    is_pglib = (
        isinstance(raw, dict)
        and "thermal_generators" in raw
        and ("time_periods" in raw or "demand" in raw)
    )
    is_native = isinstance(raw, dict) and "system" in raw

    if is_pglib:
        print(f"  Detected PGLIB-UC format → ModelData.read(..., file_type='pglib-uc')")
        return ModelData.read(path, file_type="pglib-uc")
    if is_native:
        print(f"  Detected native EGRET ModelData JSON")
        return ModelData.read(path)

    # Ambiguous: try pglib-uc then fall back
    try:
        print(f"  Format unclear; trying file_type='pglib-uc' ...")
        return ModelData.read(path, file_type="pglib-uc")
    except Exception as e1:
        print(f"  pglib-uc read failed ({type(e1).__name__}); trying default read ...")
        return ModelData.read(path)
...
def stage1_build_egret_model(
    ...
    if md is None and explicit_path is not None:
        ...
        md = _read_egret_model_data(explicit_path)
