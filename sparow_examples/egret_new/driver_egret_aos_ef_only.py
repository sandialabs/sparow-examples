#!/usr/bin/env python3
"""
driver_egret_aos_ef_only.py
===========================
Single-scenario EGRET **EF + EF AOS only** driver.

Path (no double solve):
  1. Build SP
  2. model = sp.create_EF(...)
  3. or_topas.aos.gurobi_generate_solutions(model, ...) at each rel_gap

Baseline objective is the best pool solution at the first successful AOS call.

Usage (same directory as egret_sparow_staged.py):

  python driver_egret_aos_ef_only.py --out_dir results_ef/ --intensity 3 \\
      --force-synthetic --n-periods 4 2>&1 | tee driver_ef_only.log
"""
...
def run_aos_extensive(
    ...
    """Pool search on the EF model (this is the only solve for that gap)."""
...
def main():
    parser = argparse.ArgumentParser(
        description="EGRET EF + EF AOS only (create_EF → gurobi_pool, no prior solve)",
    ...
    out_csv = out_dir / f"egret_aos_ef_only_{case}.csv"
