#!/usr/bin/env python3
"""
driver_egret_aos_multi.py
===================

EGRET + SPAROW multi-scenario AOS benchmark driver.

Target
------
- Multi-scenario stochastic UC via egret_sparow_staged_multi (load_scale × Probability).
- Default: synthetic tiny UC (2-gen copperplate, T = n_periods) with 3 scenarios.
...
------------------
  ...
  --scenario-ids low,med,high   (optional; auto-named if omitted)

Lengths of --scales and --probs must match; probabilities must sum to 1.
...
Usage
-----
  # Synthetic multi-scenario (default 3 scenarios)
  python driver_egret_aos_multi.py --out_dir results/
  python driver_egret_aos_multi.py --out_dir results/ --intensity 3 --n-periods 4
  python driver_egret_aos_multi.py --out_dir results/ --scales 0.9,1.0,1.1 --probs 0.3,0.4,0.3

  # Real EGRET JSON (horizon truncated to --n-periods with a warning)
  python driver_egret_aos_multi.py --out_dir results/ --egret_file path/to/instance.json --n-periods 4

  # Explicit first-stage list
  python driver_egret_aos_multi.py --out_dir results/ --egret_file path/to/instance.json \\
...
def build_egret_sp_multi(
    ...
    """
    Build a multi-scenario SPAROW StochasticProgram from EGRET ModelData.

    scenarios : list of dicts with keys ID, Probability, load_scale
      (same contract as egret_sparow_staged_multi.stage5_egret_sparow_sp_multi).

    first_stage_names:
      - If provided, used as-is (after Stage-3 verification).
      - Else if include_start_stop: UnitOn + UnitStart + UnitStop (heuristic).
      - Else: UnitOn only (preferred for synthetic / commitment AOS).
    """
    ...
    md, model = stage1_build_egret_model(
        force_synthetic=force_synthetic or egret_file is None,
        n_periods=n_periods,
    ...
    _print(
        f"  scenarios = "
        + ", ".join(
            f"{s['ID']}(scale={s['load_scale']}, p={s['Probability']})"
            for s in scenarios
        )
    )
...
def build_egret_sp(*args, **kwargs):
    """Backward-compat alias – prefer build_egret_sp_multi explicitly. """
    raise RuntimeError(
        "driver_egret_aos_multi requires build_egret_sp_multi(scenarios=...). "
        "Do not call build_egret_sp."
    )
...
def _case_label(
    ...
    if force_synthetic or egret_file is None:
        ...
        return f"tiny_uc_T{T}_S{n_scenarios}"
    stem = Path(egret_file).stem
    if n_periods is not None:
        return f"{stem}_T{n_periods}_S{n_scenarios}"
    return f"{stem}_S{n_scenarios}"
...
def _parse_scenarios(
    ...
    """Build scenario dicts from parallel scale / probability lists."""
    if len(scales) != len(probs):
        raise ValueError(
            f"--scales and --probs must have the same length "
            f"(got {len(scales)} scales, {len(probs)} probs)"
        )
    if len(scales) < 1:
    ...
    if any(p <= 0 for p in probs):
        raise ValueError("--probs entries must be strictly positive")
    if scenario_ids is None:
        if len(scales) == 1:
            scenario_ids = ["base"]
        elif len(scales) == 3:
            scenario_ids = ["low", "med", "high"]
        else:
            scenario_ids = [f"s{i}" for i in range(len(scales))]
    if len(scenario_ids) != len(scales):
        raise ValueError(
            f"--scenario-ids length {len(scenario_ids)} != --scales length {len(scales)}"
        )
    return [
        {"ID": sid, "Probability": float(p), "load_scale": float(sc)}
        for sid, p, sc in zip(scenario_ids, probs, scales)
    ]
...
def run_extensive_base(sp, time_limit: float) -> Dict[str, Any]:
    """
    Solve the multi-scenario extensive form; return model + objective + timing.

    Mirrors stage6_solve_extensive_form in egret_sparow_staged_multi:
      1. Prefer SPAROW ExtensiveFormSolver (compact_repn=True by default).
      2. On replace-variables / transformation-handler failure, fall back to
         sp.create_EF(compact_repn=False) + sp.solve — the multi-scenario
         control path that produces a valid reference objective.
    """
    ...
    ef_solver.set_options(solver=EF_SOLVER, solver_options=solver_options)
        ...
        try:
            ...
            if rd and "solutions" in rd and rd["solutions"]:
                ...
                if isinstance(soln, dict) and soln.get("objectives"):
                    val = float(soln["objectives"][0]["value"])
        except Exception:
            pass
        if math.isnan(val) and mdl is not None:
            try:
                objs = list(
                    mdl.component_data_objects(
                        pyo.Objective, active=True, descend_into=True
                    )
                )
                if objs:
                    val = float(pyo.value(objs[0]))
            except Exception:
                pass
        if math.isnan(val) and res is not None:
            try:
                md = getattr(res, "metadata", None)
                if md is not None:
                    for key in ("objective", "obj_value", "objective_value"):
                        if hasattr(md, "get") and md.get(key) is not None:
                            val = float(md.get(key))
                            break
                        if hasattr(md, key) and getattr(md, key) is not None:
                            val = float(getattr(md, key))
                            break
            except Exception:
                pass
        return val
    ...
    try:
        _print(
            f"  Bundles: {list(sp.bundles)}; trying ExtensiveFormSolver "
            "(compact_repn=True) ..."
        )
        if hasattr(ef_solver, "solve_and_return_EF"):
            res = ef_solver.solve_and_return_EF(sp)
        else:
            res = ef_solver.solve(sp)
        model = getattr(res, "model", None)
        obj = _extract_obj(res, model)
    except Exception as exc:
        msg = str(exc)
        is_transform = (
            "No transformation handler registered" in msg
            or "replace_variables" in msg.lower()
            or "replace-variables" in msg.lower()
        )
        if is_transform:
            _print(
            ...
            _print(
                "  Falling back to compact_repn=False "
                "(non-compact multi-scenario EF) ..."
            )
            used_compact = False
            try:
                if hasattr(sp, "set_solver"):
                    sp.set_solver(EF_SOLVER)
                M = sp.create_EF(compact_repn=False)
                res = sp.solve(M, solver_options=solver_options)
                model = M
                obj = _extract_obj(res, model)
                if math.isnan(obj):
                    try:
                        obj = float(pyo.value(M.obj)) if hasattr(M, "obj") else float("nan")
                    except Exception:
                        pass
                _print(f"  compact_repn=False EF objective: {obj}")
            except Exception as exc2:
                status = "error"
                error_msg = (
                    f"compact_repn=True: {type(exc).__name__}: {str(exc)[:150]}; "
                    f"compact_repn=False: {type(exc2).__name__}: {str(exc2)[:150]}"
                )
                import traceback
                _print("  *** EF (compact_repn=False) traceback:")
                traceback.print_exc()
                return {
                    "model": None,
            ...
            return {
                ...
                "status": status,
                "error_msg": error_msg,
                "used_compact": used_compact,
            }

    elapsed = time.perf_counter() - t0
    if elapsed > time_limit + 1.0 and status == "optimal":
        status = "timeout"
    if status == "optimal" and math.isnan(obj):
        status = "error"
        error_msg = "EF solve returned without a finite objective value"

    _print(
        f"  EF done (compact={used_compact}): status={status}, obj={obj}, "
        f"time={elapsed:.2f}s"
    )
    return {
        "model": model,
...
def run_benders_base(
    ...
    # same SP; we still strip anything that still looks dangerous across
    # every multi-scenario bundle.
    n_stripped = 0
    try:
        candidates = []
        for b_name in list(getattr(sp, "bundles", []) or []):
            for getter in (
                lambda b=b_name: sp.get_model(b) if hasattr(sp, "get_model") else None,
                lambda b=b_name: sp.s[None, b] if hasattr(sp, "s") else None,
                lambda b=b_name: getattr(sp, "scenarios", {}).get(b),
            ):
                try:
                    m = getter()
                    if m is not None:
                    ...
                    pass
        # Also walk any Block that looks like a scenario model on the SP
        try:
            for blk in sp.component_data_objects(pyo.Block, descend_into=True):
                if hasattr(blk, "Contingencies") or hasattr(blk, "UnitOn"):
        ...
        if n_stripped:
            _print(
                f"  Stripped {n_stripped} Contingencies component(s) "
                "before Benders deepcopy"
            )
        else:
            _print("  (No Contingencies components found to strip)")
...
def run_baseline_extensive(
    *,
    ...
    try:
        sp, fs_names = build_egret_sp_multi(
            n_periods=n_periods,
            include_start_stop=include_start_stop,
            scenarios=scenarios,
            egret_file=egret_file,
...
def main():
    ...
    parser.add_argument(
        "--scales",
        type=str,
        default="0.85,1.00,1.15",
        help="Comma-separated load scales (one per scenario). Default: 0.85,1.00,1.15",
    )
    parser.add_argument(
        "--probs",
        type=str,
        default="0.25,0.50,0.25",
        help="Comma-separated scenario probabilities (must sum to 1). Default: 0.25,0.50,0.25",
    )
    parser.add_argument(
        "--scenario-ids",
        type=str,
        default=None,
        help="Comma-separated scenario IDs (optional; auto-named if omitted)",
    )
    parser.add_argument(
        "--aos-methods",
        nargs="+",
    ...
    # Parse multi-scenario definition
    ...
    scenarios = _parse_scenarios(scales, probs, ids)
    ...
    case = _case_label(
        args.egret_file, args.n_periods, args.force_synthetic, n_scenarios=len(scenarios)
    )
    ...
    _print(f"n_scenarios = {len(scenarios)}")
    for s in scenarios:
        _print(f"  scenario {s['ID']}: scale={s['load_scale']}, p={s['Probability']}")
    ...
    for include_ss in first_stage_modes:
        ...
        # ------------------------------------------------------------------
        _print(f"Building multi-scenario SP + baseline extensive  mode={fs_label} ...")
        sp, fs_names = build_egret_sp_multi(
        ...
        base_row["n_scenarios"] = len(scenarios)
        ...
        if cached_ef["status"] == "error" and cached_ef.get("error_msg"):
            _print(f"  *** EF base error: {cached_ef['error_msg']}")

        _print(f"Benders base solve (once, reused for all Benders AOS gaps) ...")
        # CRITICAL (multi-scenario): do NOT reuse the EF SP.
        # SPAROW's non-compact EF path (create_EF(compact_repn=False)) mutates
        # bundling so that len(bundle.scenarios) != 1, which then fails the
        # assertion in BendersSolver._transform_to_master_model.
        # Multi Stage 8 explicitly rebuilds a fresh SP after EF for this reason
        # (see run_staged_diagnostics_multi). Contingencies on a fresh multi SP
        # only warns ("Setting field to None") — it does not hard-crash.
        _print("  Rebuilding fresh multi-scenario SP for Benders (EF mutates bundling)")
        sp_b, _ = build_egret_sp_multi(
            n_periods=args.n_periods,
            include_start_stop=include_ss,
            scenarios=scenarios,
            egret_file=args.egret_file,
        ...
        # Sanity: one scenario per bundle is required by SPAROW Benders
        for _b in sp_b.bundles:
            _sc = getattr(sp_b.bundles[_b], "scenarios", None)
            _n = len(_sc) if _sc is not None else None
            _print(f"  bundle {_b!r}: n_scenarios={_n}")
            if _n is not None and _n != 1:
                _print(
                    f"  WARNING: bundle {_b!r} has {_n} scenarios; "
                    "SPAROW Benders requires exactly 1"
                )
