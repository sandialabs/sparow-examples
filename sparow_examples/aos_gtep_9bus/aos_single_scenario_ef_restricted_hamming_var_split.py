...
def is_target_gen_indicator(name: str) -> bool:
    n = (name or "").lower()
    return (
        "investmentstage" in n
        and "binary_indicator" in n
        and any(p in n for p in TARGET_PATTERNS)
    )

# after the existing fs_name_set / is_first_stage block and after
# b0 = next(iter(sp.bundles))

if hasattr(sp, "int_to_FirstStageVar") and b0 in sp.int_to_FirstStageVar:
    candidates = list(sp.int_to_FirstStageVar[b0].values())
else:
    # defensive fallback
    candidates = list(
...
variables = [
    ...
    if v.is_binary() and not v.is_fixed() and is_target_gen_indicator(v.name)
]

print(f"  AOS restricted to {len(variables)} free gen* binary indicators")
for v in sorted(variables, key=lambda x: x.name)[:8]:
    print(f"    {v.name}")
