...
def plot_objective_values(mod_object, filename="investment.png"):
    OC_I1 = pyo.value(mod_object.investmentStage[1].operatingCostInvestment)
    OC_I2 = pyo.value(mod_object.investmentStage[2].operatingCostInvestment)
    OC_I3 = pyo.value(mod_object.investmentStage[3].operatingCostInvestment)

    ic_I1 = pyo.value(mod_object.investmentStage[1].investment_cost)
    ic_I2 = pyo.value(mod_object.investmentStage[2].investment_cost)
    ic_I3 = pyo.value(mod_object.investmentStage[3].investment_cost)
...
def plot_battery_investments(mod_object):
    ins = []
    oper = []
    retir = []
    dis = []
    ...
    for g in mod_object.investmentStage[1].storInstalled.keys():
        ins.append(
            pyo.value(
                mod_object.investmentStage[1]
                .storInstalled[g]
        ...
        retir.append(
            pyo.value(
                mod_object.investmentStage[1].storRetired[g].binary_indicator_var
            )
        )
        dis.append(
            pyo.value(
                mod_object.investmentStage[1].storDisabled[g].binary_indicator_var
            )
        )
        ext.append(
            pyo.value(
                mod_object.investmentStage[1].storExtended[g].binary_indicator_var
            )
        )
    print(ins)
    print(oper)
    ...
    dis_2 = []
    ext_2 = []
    for g in mod_object.investmentStage[2].storInstalled.keys():
        ins_2.append(
            pyo.value(
                mod_object.investmentStage[2]
                .storInstalled[g]
                .binary_indicator_var
            )
        )
        ...
        retir_2.append(
            pyo.value(
                mod_object.investmentStage[2].storRetired[g].binary_indicator_var
            )
        )
        dis_2.append(
            pyo.value(
                mod_object.investmentStage[2].storDisabled[g].binary_indicator_var
            )
        )
        ext_2.append(
            pyo.value(
                mod_object.investmentStage[2].storExtended[g].binary_indicator_var
            )
        )
    print(ins_2)
    print(oper_2)
    ...
    dis_3 = []
    ext_3 = []
    for g in mod_object.investmentStage[3].storInstalled.keys():
        ins_3.append(
            pyo.value(
                mod_object.investmentStage[3]
                .storInstalled[g]
                .binary_indicator_var
            )
        )
        ...
        retir_3.append(
            pyo.value(
                mod_object.investmentStage[3].storRetired[g].binary_indicator_var
            )
        )
        dis_3.append(
            pyo.value(
                mod_object.investmentStage[3].storDisabled[g].binary_indicator_var
            )
        )
        ext_3.append(
            pyo.value(
                mod_object.investmentStage[3].storExtended[g].binary_indicator_var
            )
        )
    print(ins_3)
    print(oper_3)
    ...
    generators_stage_1 = list(mod_object.investmentStage[1].storInstalled.keys())
    generators_stage_2 = list(mod_object.investmentStage[2].storInstalled.keys())
    generators_stage_3 = list(mod_object.investmentStage[3].storInstalled.keys())
    bar_width = 0.2
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("plots/"+"Bat_Invest_1.png")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("plots/"+"Bat_Invest_2.png")
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("plots/"+"Bat_Invest_3.png")
    return

def sum_all_load_shed_new(mod_object):
    total = 0
    cost_dispatch_total = 0
    invest_stages = mod_object.investmentStage.keys()

    for IS in invest_stages:
        rep_stages = mod_object.investmentStage[IS].representativePeriod.keys()

        for rp in rep_stages:
            com_stages = (
                mod_object.investmentStage[IS]
...
def plot_load_shed_generation_bar_plot(mod_object):
    storage_charged = []
    storage_discharged = []
    load_shed = []
    thermal_generation = []
    ...
    plt.savefig("plots/"+"load_shed.png")
    return

def plot_load_profiles(mod_object):
    # Step 1: Retrieve load values for each bus
    bus_loads = {}
    for bus in mod_object.data.md.data["elements"]["load"].keys():
        bus_loads[bus] = mod_object.data.md.data["elements"]["load"][bus][
            "p_load"
        ]["values"]
    ...
    plt.savefig("plots/"+"Load_Demand.png")

    return

def plot_objective_cats_by_dispatch(mod_object):
    cost_dispatch = []
    ...
    # Step 5: Show the plot
    plt.savefig("plots/"+"Costing_categories.png")
    return

def plot_bar_broad_cats(mod_object):
    storage_charged = []
    storage_discharged = []
    load_shed = []
    thermal_generation = []
    ...
    for i in range(0, len(load_shed), 24):
        ...
        plt.savefig("plots/"+str(i) + "bar_plot_broad_cat.png")
        # Loop through the data in chunks of 24
    for i in range(0, len(load_shed), 24):
        # Set up the bar width
        bar_width = 0.5  # Width of the bars
        ...
        # Show the plot for the current chunk
        plt.savefig("plots/"+str(i) + "bar_plot_all_cat.png")
    for i in range(0, len(load_shed), 24):
        # Set up the bar width
        bar_width = 0.5  # Width of the bars
        # Extract the current chunk
        ...
        plt.tight_layout()  # Adjust layout to prevent clipping of labels
        plt.savefig("plots/"+str(i) + "stack_plot_all_cat.png")

    return


def post_process(mod_object):
    os.makedirs("plots", exist_ok=True)
    # -----PLOT 1-------#
    plot_objective_values(mod_object)
    # -----PLOT 2-------#
    plot_battery_investments(mod_object)
    # -----PLOT 3/LOAD SHED INFO-------#
    total = sum_all_load_shed_new(mod_object)
    print("Total Load Shed Value:")
    print(total)
    # -----PLOT 4-------#
    plot_load_shed_generation_bar_plot(mod_object)
    # -----PLOT 5-------#
    plot_load_profiles(mod_object)
    # -----PLOT 6-------#
    plot_objective_cats_by_dispatch(mod_object)
    # -----PLOT 7-------#
    plot_bar_broad_cats(mod_object)
